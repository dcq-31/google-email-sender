...
(Operational variables — DB DSN, SMTP credentials, worker poll interval/batch — are in
...
| C4 | Sender identity | Single shared sender (env-configured SMTP account + App Password) for v1. |
| C5 | `nextAttemptAt` filter | `pending AND nextAttemptAt <= now` (inclusive). |

## 9. Out-of-scope risks / future work

- Per-tenant SMTP credentials and per-tenant send quotas.
